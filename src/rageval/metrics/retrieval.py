...
def hit_rate_at_k(example: EvaluationExample, contexts: list[RetrievedContext], k: int) -> MetricResult:
...
def recall_at_k(example: EvaluationExample, contexts: list[RetrievedContext], k: int) -> MetricResult:
    expected = set(example.expected_document_ids)
    if not expected:
...
def precision_at_k(example: EvaluationExample, contexts: list[RetrievedContext], k: int) -> MetricResult:
    expected = set(example.expected_document_ids)
    if not contexts[:k]:
